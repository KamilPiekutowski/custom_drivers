...
ssize_t label_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
  struct gpiodev_private_data *dev_data = dev_get_drvdata(dev);
  return sprintf(dev_data->label, "%s\n", buf);
}
...
int gpio_sysfs_probe(struct platform_device *pdev)
{
  ...
  for_each_available_child_of_node(parent,child)
  {
    ...
    /* default: set gpio direction to output*/
    ret = gpiod_direction_output(dev_data->desc,0);
    if(ret)
    {
